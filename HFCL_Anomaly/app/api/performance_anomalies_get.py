...
class AnalysisSummary(BaseModel):
    total_overloaded_aps: int
    summary_message: str
    highest_client_ap: str
    highest_util_ap: str
...
class APOverloadOptimizedResponse(BaseModel):
    status: str = "success"
...
# --- API Endpoint ---
@router.post("/ap-overload/analyze", response_model=APOverloadOptimizedResponse)
async def analyze_ap_overload(request: APOverloadRequest):
    ...
    try:
        ...
        # Stage 2: Identify Overloaded and Underutilized APs
        ...
        # Create a set of overloaded AP names for efficient lookup
        ...
        # Stage 4: Generate a high-level summary
        total_overloaded_aps = len(overloaded_aps_df)
        if total_overloaded_aps == 0:
            summary_message = "No APs are currently overloaded based on the specified thresholds."
        elif len(recommendations) == 0:
            summary_message = "Widespread client density issue. No suitable neighbors found for load balancing."
        else:
            summary_message = f"{total_overloaded_aps} APs are overloaded. Recommendations are available for some."
        
        highest_client_ap = ""
        highest_util_ap = ""
        if not ap_summary.empty:
            highest_client_ap = ap_summary.sort_values(by='client_count', ascending=False).iloc[0]['ap_name']
            highest_util_ap = ap_summary.sort_values(by='avg_ch_util', ascending=False).iloc[0]['ap_name']

        analysis_summary = AnalysisSummary(
            total_overloaded_aps=total_overloaded_aps,
            summary_message=summary_message,
            highest_client_ap=highest_client_ap,
            highest_util_ap=highest_util_ap
        )
        ...
        return APOverloadOptimizedResponse(
